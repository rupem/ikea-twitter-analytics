...
with open('batch_processing\\User_details.yaml') as f:
...
logging.info("**********tweets for the hashtag/keyword "+sys.argv[1]+" for the date "+str(date.today())+" are loading **************")

query = sys.argv[1]+' -is:retweet lang:en'
...
df.to_csv("batch_processing\\data\\raw_data\\"+str(date.today())+".csv", sep ='\t')
logging.info("********** Loaded file tweets into batch_processing\\data\\raw_data\\"+str(date.today())+".csv **************")
